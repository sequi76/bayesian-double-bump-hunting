...
__all__ = [
    "runplot", "traceplot", "traceplot2", "cornerpoints", "cornerplot", "boundplot",
    "cornerbound", "_hist2d"
]
...
"""
    ...
    if truths is not None:
        try:
            ...
            curt = [truths]
        for t in curt:
            func(t, color=truth_color, **truth_kwargs, label='true' )
...
def traceplot(results,
    ...
    for i, x in enumerate(samples):
        ...
        sf = ScalarFormatter(useMathText=use_math_text)
        ax.yaxis.set_major_formatter(sf)
        ax.set_xlabel(r"$-\ln {\cal X}$", **label_kwargs)
        ax.set_ylabel(labels[i], **label_kwargs)
        ...
        if isinstance(s, int_type):
            ...
            x0 = 0.5 * (b[1:] + b[:-1])
            y0 = n
            ax.fill_between(x0, y0, color=color, **post_kwargs, label='posterior')
        ...
        # my priors
        if i < 6:
            ddd = np.linspace(means[i][0]-3*means[i][1], means[i][0]+3*means[i][1], 100)
            ax.plot(ddd, norm.pdf(ddd, loc=means[i][0], scale=means[i][1] ), color='black', linestyle='dashed', label='prior')
        else:
            ax.axhline(y=1.0, color='black', linestyle='dashed', label='prior')
            ax.set_xlim(0,1)
...
def traceplot2(results,
    ...
    # Setting up default plot layout.
    fig, axes = _make_subplots(fig, 1, 1, 6, 3)
